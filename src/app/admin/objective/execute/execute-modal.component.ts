...
import { OKRServiceProxy, Status, Category, Charge, CreateObjectiveInput, UpdateObjectiveInput, AddOrUpdateKeyResultInput, AddOrUpdateExcuteInput } from '@shared/service-proxies/service-proxies-okr';

@Component({
    selector: 'executetModal',
...
export class CreateOrEditExecuteModalComponent extends AppComponentBase implements AfterViewChecked {
    ...
    show(objItem?: any): void {
        ...
        if (objItem) {
            ...
            this._OKRServiceProxy.getUsersForExcute(this.keyResultId)
            .subscribe(r => {
                (r || []).forEach(e => {
    ...
    save(): void {
        this.objItem.keyResultId = this.keyResultId
        this.saving = true;
        ...
        if (!this.objItem.id) {
            this._OKRServiceProxy.addOrUpdateExcute(new AddOrUpdateExcuteInput(this.objItem))
